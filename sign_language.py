    ...
    if results.multi_hand_landmarks:
        for hand_landmark in results.multi_hand_landmarks:
            ...
            for tip in finger_tips:
                x, y = int(lmList[tip].x * w), int(lmList[tip].y * h)
            ...
            #checking if the thumb is up
                if lm_list[thumb_tip].y < lm_list[thumb_tip-1].x - lm_list[thumb_tip-2].y:
                    print("LIKE")
                    cv2.putText(img, "LIKE", (20,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 3)
                    #check if thumb is down
                    if lm_list [thumb_tip].y > lm_list [thumb_tip-1].y > lm_list[thumb_tip-2].y: print("DISLIKE")
